(function(global){
	...
	function tagLoop(node) {
		...
		code += "} ";
		code += "})(G.clone($),G.clone(_),G.clone($$)); ";
		code += templateErrorGuard("ret","ret2");
	...
	function tagIncludeTemplate(node) {
		...
		code += "try { ";
		code += "ret2 = G.include(" + expr(node.main_expr) + ",$); ";
		code += "} catch (err) { ";
	...
	function children(node) {
		...
		for (i=0; i<node.children.length; i++) {
			...
			if (child.type === global.grips.parser.TEXT) {
				code += "ret += \"" + escapeNewlines(escapeDoubleQuotes(child.val)) + "\"; ";
			}
			else if (child.type === global.grips.parser.TAG_RAW) {
				code += "ret += \"" + escapeNewlines(escapeDoubleQuotes(child.def[0].val)) + "\"; ";
			}
