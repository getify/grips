(function __grips_generator__(_Grips){
	...
	function startCollection(node) {
		...
		code += "extend = G.extend, error = G.error, ";
		code += "cID = \"" + node.start + "\"; ";
		return code;
	}
	...
	function expr(node) {
		...
		if (node.type === _Grips.parser.STRING_LITERAL ||
			...
			throw new _Grips.parser.ParserError("Invalid EXPR",node);
		}
	...
	function tagDefine(node) {
		...
		code += children(node);
		code += "return ret; ";
		code += "},\"" + node.id.val + "\"," + simpleNodeJSON(node) + "); ";
		return code;
	}
	...
	function tagLoop(node) {
		...
		if (node.main_expr.def[0].type === _Grips.parser.SET_LITERAL) {
			...
			code += "}} ";
			code += "} else { ";
			code += "return error(cID," + simpleNodeJSON(node.main_expr) + ",\"Invalid loop-iterator reference\"); ";
			code += "} ";
		}
