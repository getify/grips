(function(global){
	...
	function process(chunk,filename) {
		...
		function handleOutsideMatch() {
			...
			parser_res = global.grips.parser.nodify(tokensSlice,filename);
		...
		function handleLiteralMatch(){
			...
			if (match) {
				...
				if (!leftContext || leftContext.match(not_escaped_pattern)) {
					...
					parser_state_patterns[global.grips.parser.STRING_LITERAL] = null;
				}
				// otherwise just add the match to the literal's general content token
				else {
					// was it escaped?
		...
		// if we aren't given a `filename` to associate the parsing with, substitute a random unique chunk ID
		if (!filename) {
			filename = generateNewChunkID();
			chunk_ids[filename] = true;
			filename = "chunk_" + filename;
		}
