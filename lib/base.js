...
/* STOP_DEBUG */
		...
		function definePartial(fn,id/* START_DEBUG */,obj/* STOP_DEBUG */) {
			...
			collections[collection_id].partials[id.replace(/^.*#/,"#")] = function __handle_partial__(){
				var /* START_DEBUG */_err,/* STOP_DEBUG */ ret;
...
/* STOP_COMPILER */
		...
		function render(id,$,$$) {
			...
			// collection ID just plain missing
				...
				throw /* START_DEBUG */new TemplateError("Required collection ID missing: " + id) ||/* STOP_DEBUG */unknown_error;
			}
			...
			if (collection_id in collections) {
				...
				for (i=0; i<(render_partials_stack.length-1); i++) {
					if (render_partials_stack[i] === tmp) {
						throw /* START_DEBUG */new TemplateError("Recursive template include: " + tmp) ||/* STOP_DEBUG */unknown_error;
					}
				}
