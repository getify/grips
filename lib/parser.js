(function(global){
	...
	function parse(node) {
		function validateTagID(node) {
			var tmp;
			if ((tmp = node.val.match(/#/g)) && tmp.length > 1) {
				return new ParserError("Unexpected extra #id",node);
			}
			else if (node.parent.type === NODE_TAG_DEFINE) {
				if (!(node && node.val && node.val.match(/#[a-z0-9_\-$]+$/i))) {
					return new ParserError("Expected #id",node);
				}
				else if (!node.val.match(/^#/)) {
					return new ParserError("Unexpected text before #id",node);
				}
				// need to add the filename to the Define Tag ID
				else {
					node.val = node.filename + node.val;
					delete node.filename;
				}
			}
			else if (node.parent.type === NODE_TAG_EXTEND) {
				if (node.val.match(/^#/)) {
					return new ParserError("Expected template filename",node);
				}
			}
		...
		if (node.type === NODE_ID) {
		...
		else if (node.type === NODE_TAG_EXTEND) {
			...
			if (node.decl && node.decl.length) {
				...
				if (node.decl.length > 1) {
					console.log(JSON.stringify(node.decl));
					for (var i=1; i<node.decl.length; i++) {
						if (node.decl[i].type !== NODE_WHITESPACE) {
							console.log(node.decl[i]);
							throw new ParserError("Unexpected",node.decl[i]);
						}
					}
				}
		...
		else if (node.type === NODE_DEFINE_EXPR) {
			return node;
		}
