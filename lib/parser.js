(function(global){
	...
	function parse(node) {
			...
			if (expr.decl && expr.decl.length > 0) {
				for (i=0; i<expr.decl.length; i++) {
					...
					// otherwise, handle RH of assignment statement
						...
						rh_expr = identifyConditionalExpr(expr.decl.slice(i));
		...
		function filterOperators(nodes) {
