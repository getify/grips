(function(global){
	...
	Node.prototype.toString = function() {
		...
		if (this.type === NODE_TAG_DEFINE) {
			...
			ret = this.decl[0].toString() + " = " + this.decl[1].toString()
		}
		else if (this.type === NODE_MAIN_REF_EXPR) {
			ret = showDeclaration(this);
		}
		else if (this.type === NODE_CONDITIONAL_EXPR) {
			ret = this.decl[0].toString() + " ? " + this.decl[1].toString() + " : " + this.decl[2].toString();
		}
			...
			ret = showLiteral(this);
		}
		else if (this.type === NODE_RANGE_LITERAL) {
			ret = "[" + this.decl[0] + ".." + this.decl[1] + "]";
		}
		else if (this.type === NODE_SET_LITERAL) {
			ret = "";
			for (var i=0; i<this.decl.length; i++) {
				ret += (ret !== "" ? "," : "") + this.decl[i].val;
			}
			ret = "[" + ret + "]";
		}
		...
		else if (this.type === NODE_OPERATOR) {
			ret = this.val;
		}
				...
				if (current_parent) {
					...
					if (current_parent.type === NODE_GENERAL_EXPR ||
						...
						if (token.val === "-" &&
							...
							if (current_parent.decl[current_parent.decl.length-1].type !== NODE_WHITESPACE &&
								!(
									current_parent.decl[current_parent.decl.length-1].type === NODE_OPERATOR &&
									current_parent.decl[current_parent.decl.length-1].val.match(/(?:\.\.)|\[/)
								)
							) {
								return new global.grips.tokenizer.TokenizerError("Unexpected token",token);
							}
				...
				if (current_parent) {
					...
					if (current_parent.type === NODE_GENERAL_EXPR ||
						...
						if (token.val.match(/^\d+$/) &&
							current_parent.decl.length > 0 &&
							current_parent.decl[current_parent.decl.length-1].type === NODE_OPERATOR &&
							current_parent.decl[current_parent.decl.length-1].val === "-"
						) {
							current_parent.decl[current_parent.decl.length-1] = new Node({
								parent: current_parent,
								type: NODE_TEXT,
								token: new global.grips.tokenizer.Token({
	...
	function parse(node) {
		...
		function identifyConditionalExpr(nodes) {
			function processExpr(decl,type,validateFn) {
				var expr, i;

				decl = stripOuterGroupings(decl);
			...
			if (nodes.length > 0) {
					...
					if (node.type === NODE_OPERATOR &&
						...
						if (node.val === "?") {
							if (!conditional_expr) {
								...
								test_expr = processExpr(decl,NODE_BOOLEAN_EXPR,validateRHExpr);
							...
							if (conditional_expr && !then_expr) {
								then_expr = processExpr(decl,NODE_VAL_EXPR,validateValueExpr);
				...
				if (decl.length > 0) {
					if (conditional_expr) {
						if (!then_expr) {
							...
							else_expr = processExpr(decl,NODE_VAL_EXPR,validateValueExpr);
				...
				if (test_expr && then_expr) {
					conditional_expr.decl.push(test_expr,then_expr,else_expr);
		...
		function validateTagID(id) {
			...
			if ((tmp = id.val.match(/#/g)) && tmp.length > 1) {
				...
				if (!id.val) {
					...
					return new global.grips.tokenizer.TokenizerError(
						"Unexpected token",
						new global.grips.tokenizer.Token({
							type: global.grips.tokenizer.GENERAL,
							val: tmp[2],
		...
		function validateRHExpr(expr) {
			...
			if (expr.decl && expr.decl.length > 0) {
				for (i=0; i<expr.decl.length; i++) {
					...
					if (expr.decl[i].type === NODE_WHITESPACE) {
						continue;
					}
					...
					if (node.type === NODE_OPERATOR) {
						...
						if (prev_node) {
							...
							if (prev_node.type === NODE_OPERATOR) {
								if (node.val === "(" &&
									...
									return new ParserError("Invalid EXPR",node);
								}
								else if (node.val === "!" &&
									!prev_node.val.match(/[!\(]/)
								) {
						...
						if (node.val.match(/[\[\(]/)) {
							...
							if (stack.length > 0 &&
								...
								return new ParserError("Unbalanced EXPR",node);
							}
						}
					}
						...
						if (prev_node &&
							...
							return new ParserError("Unexpected string literal",node);
						}
					}
				}
				if (stack.length > 0) {
					return new ParserError("Unbalanced EXPR",expr);
				}
			}
			else {
		...
		function validateRangeExpr(expr) {
			if (expr.decl && expr.decl.length === 5) {
				if (!(
						expr.decl[1].type === NODE_TEXT &&
						expr.decl[1].val.match(/^-?\d+$/)
					)
				) {
					return new ParserError("Invalid range literal",expr.decl[1]);
				}
				else if (!(
						expr.decl[3].type === NODE_TEXT &&
						expr.decl[3].val.match(/^-?\d+$/)
					)
				) {
					return new ParserError("Invalid range literal",expr.decl[3]);
				}
				else if (!(
						expr.decl[2].type === NODE_OPERATOR &&
						expr.decl[2].val === ".."
					)
				) {
					return new ParserError("Invalid range literal",expr.decl[2]);
				}
			}
			else {
				return new ParserError("Expected range literal",expr);
			}
		}
		...
		function validateSetExpr(expr) {
			...
			if (expr.decl && expr.decl.length >= 5) {
				for (i=1; i<expr.decl.length-1; i++) {
					prev_node = node;
					node = expr.decl[i];
					...
					if (node.type === NODE_STRING_LITERAL) {
						if (prev_node &&
							...
							return new ParserError("Invalid set literal",node);
						}
					}
				...
				if (expr.decl[expr.decl.length-2].type !== NODE_STRING_LITERAL) {
					return new ParserError("Invalid set literal",expr.decl[expr.decl.length-2]);
				}
			}
			else {
				return new ParserError("Expected set literal",expr);
			}
		}

		function validateAssignmentExpr(expr) {
			var i, j, prev_node, node, tmp, tmp2, decl = [],
				lh_expr, rh_expr
			;
			...
			if (expr.decl && expr.decl.length > 0) {
				for (i=0; i<expr.decl.length; i++) {
					...
					if (!lh_expr) {
						if (node.type === NODE_OPERATOR) {
							if (node.val === "=") {
								...
								if (!(
										decl[0] &&
										decl[0].type === NODE_REF_EXPR &&
										decl[1] &&
										decl[1].type === NODE_SET_LITERAL
									)
								) {
								...
								// remap `parent`s
								for (j=0; j<lh_expr.decl.length; j++) {
									lh_expr.decl[j].parent = lh_expr;
								}
								...
								tmp = identifyBracketLiteral(expr.decl.slice(i));
								if (tmp) {
									...
									tmp2 = new Node({
										parent: null,
										type: NODE_REF_EXPR,
										token: expr.decl[0].token,
										decl: expr.decl.slice(0,i)
									});
									...
									if (!err) {
										...
										tmp2 = new Node({
											parent: null,
											type: NODE_SET_LITERAL,
											token: node.token,
											decl: tmp
										});
										err = validateSetExpr(tmp2);
										// is the set-literal expression valid?
										if (!err) {
											decl.push(tmp2);

											// remove operators from the set-literal
											discardOperators(tmp2.decl);
										}
									}
								...
								return new ParserError("Invalid statement EXPR",node);
							}
							else {
								decl.push(node);
							}
					...
					// otherwise, handle RH of assignment statement
						...
						tmp = stripOuterGroupings(expr.decl.slice(i));

						if (tmp.length === 0) {
							return new ParserError("Expected EXPR",node);
						}
						...
						// remap `parent`s
						for (j=0; j<rh_expr.decl.length; j++) {
							rh_expr.decl[j].parent = rh_expr;
						}
				...
				if (lh_expr && rh_expr) {
					lh_expr.decl = stripWhitespace(lh_expr.decl);
					rh_expr.decl = stripWhitespace(rh_expr.decl);

					// update original node with statement's parsed structure
					expr.decl = [lh_expr,rh_expr];
					expr.type = NODE_ASSIGNMENT_EXPR;
				}
				else return new ParserError("Invalid assignment statement",expr);
			}
			else {
				return new ParserError("Expected assignment statement",expr);
			}
		}
		...
		if (node.type === NODE_TAG_EXTEND) {
			...
			if (node.decl && node.decl.length) {
				ret = [node.id];
				ret3 = stripWhitespace(node.decl);
				for (i=1; i<ret3.length; i++) {
					ret2 = parse(ret3[i]);
			...
			if (node.decl && node.decl.length) {
				...
				node.decl = stripWhitespace(node.decl);
				...
				throw new ParserError("Expected EXPR",node);
			}
			...
			if (node.decl && node.decl.length) {
				...
				ret = [];
				for (i=0; i<node.decl.length; i++) {
					if (node.decl[i].type !== NODE_WHITESPACE) {
						ret.push(node.decl[i]);
					}
				}
				node.decl = stripOuterGroupings(ret);

				// can we infer that this expression is a bracket literal type?
				if ((ret = identifyBracketLiteral(node.decl)) &&
					ret.length === node.decl.length
				) {
					ret = new Node({
						parent: node,
						type: null,
					...
					// remap `parent`s
					for (i=0; i<ret.decl.length; i++) {
						ret.decl[i].parent = ret;
					}
					...
					if (err) {
						throw new ParserError("Invalid bracket literal",node);
					}
					else {
						// remove operators from the bracket-literal
						discardOperators(ret.decl);
