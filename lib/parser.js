(function(global){
	...
	function nodify(tokens) {
		...
		function handleInsideState(token) {
			if (current_parent && current_parent.type == null && token.type !== global.grips.tokenizer.SIGNIFIER) {
				instance_api.state = NODE_STATE_INVALID;
				return new ParserError("Expected Tag type-signifier",token);
			}
			else if (token.type === global.grips.tokenizer.SIGNIFIER) {
