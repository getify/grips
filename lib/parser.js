(function(global){
	...
	Node.prototype.toString = function() {
		...
		if (this.type === NODE_TAG_DEFINE) {
			...
			ret = "{$= @";
			if (this.main_expr) {
				ret += this.main_expr.toString().replace(/(["']).*#/,"$1#");
	...
	function nodify(tokens,filename) {
		...
		// need to insert a start filename marker into the node stream?
		if (filename !== current_filename) {
			// need to close a current filename marker?
			if (current_filename) {
				nodes.push(new Node({
					type: NODE_FILEMARKER,
	...
	function parse(node) {
		...
		function validateRHExpr(expr) {
			function inLoopScope(node) {
				while (node.parent && (node = node.parent)) {
					if (node.type === global.grips.parser.TAG_LOOP) {
						return true;
					}
				}
				return false;
			...
			function inMainRef(node) {
				while (node.parent && (node = node.parent)) {
					if (node.type === global.grips.parser.MAIN_REF_EXPR ||
						node.type === global.grips.parser.ASSIGNMENT_EXPR
					) {
						return (node.type === global.grips.parser.MAIN_REF_EXPR);
					}
				}
				return false;
			}
			...
			if (expr.def && expr.def.length > 0) {
				for (i=0; i<expr.def.length; i++) {
					...
					if (node.type === NODE_OPERATOR) {
						...
						if (node.val.match(/^\d/)) {
							if (!node.val.match(/^\d+$/)) {
								return new ParserError("Invalid identifier in EXPR",node);
							}
							else if (prev_node &&
								!(
								...
								return new ParserError("Unexpected number",node);
							}
						}
						tmp = inMainRef(node) || (node.parent.parent.context_expr === node.parent);
						...
						if (node.val === "$$" &&
							...
							return new ParserError("Unexpected Identifier in EXPR",node);
						}
						if (prev_node) {
		...
		function validateRangeExpr(expr) {
			if (expr.def && expr.def.length === 5) {
				if (!(
					...
					return new ParserError("Invalid range literal",expr.def[1]);
				}
				else if (!(
					...
					return new ParserError("Invalid range literal",expr.def[3]);
				}
				else if (!(
					...
					return new ParserError("Invalid range literal",expr.def[2]);
				}
			}
			else {
		...
		function validateSetExpr(expr) {
			...
			if (expr.def && expr.def.length >= 5) {
				...
				if (expr.def[expr.def.length-2].type !== NODE_STRING_LITERAL) {
					return new ParserError("Invalid set literal",expr.def[expr.def.length-2]);
				}
			}
			else {
								...
								if (tmp) {
									...
									if (!err) {
										...
										if (!err) {
											...
											err = new ParserError("Invalid bracket literal",node);
										}
									}
		...
		if (node.type === NODE_TAG_EXTEND) {
				...
				if (node.def.length > 1) {
					for (i=1; i<node.def.length; i++) {
						if (node.def[i].type !== NODE_WHITESPACE) {
							throw new ParserError("Unexpected",node.def[i]);
						}
					}
			...
			if (!(node.main_expr && (ret = parse(node.main_expr)))) {
				throw new ParserError("Expected template reference EXPR for Tag",node);
			}
			node.main_expr = ret;
	...
	instance_api = {
		...
		FILEMARKER: NODE_FILEMARKER,

		state: NODE_STATE_OUTSIDE,

		nodify: nodify,
		endCurrentStream: endCurrentStream,
